...
class yg_actxrocketModule(loader.Module):
    ...
    async def watcher(self, message):
        if message.text and 'https://t.me/xrocket?start=' in message.text:
            match = re.search(r'https://t.me/xrocket\?start=([A-Za-z0-9_/]+)', message.raw_text)
            if match:
                code = match.group(1)
        ...
        if message.reply_markup and message.reply_markup.rows:
            for row in message.reply_markup.rows:
                for button in row.buttons:
                    if button.url and button.url.startswith("https://t.me/xrocket?start="):
    ...
    def extract_text_from_button_url(url):
        pattern = r"https://t.me/xrocket\?start=([A-Za-z0-9_/]+)"
    ...
    async def send_text_to_tonRocketBot(self, text):
        await self.client.send_message("xrocket", f"/start {text}")

    async def stat_ygcmd(self, message):
        """проверить работоспособность"""
        await message.edit("<b>Активатор чеков @xrocket работает <emoji document_id=5348140027698227662>🙀</emoji></b>")
